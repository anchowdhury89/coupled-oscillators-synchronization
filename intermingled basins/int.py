...
def ry1f(x1,y1,x2,y2,t):
  g = w*t
  return -a*y1-b*(x1-x1**3)+f*cos(g)-p*(x1-x2)-q*(x1**2-x2**2)
def rx2f(x1,y1,x2,y2,t):
  return y2
def ry2f(x1,y1,x2,y2,t):
  g = w*t
  return -a*y2-b*(x2-x2**3)+f*cos(g)-p*(x2-x1)-q*(x2**2-x1**2)
h = 0.01
n = 500000
x = zeros((n,5))
x[0,0]=0
...
for i in range (0,n-1):
  ...
  k4y1 = ry1f(x[i,0]+.5*h*k3x1,x[i,1]+.5*h*k3y1,x[i,2]+.5*h*k3x2,x[i,3]+.5*h*k3y2,i*h+h)
  k4x2 = rx2f(x[i,0]+.5*h*k3x1,x[i,1]+.5*h*k3y1,x[i,2]+.5*h*k3x2,x[i,3]+.5*h*k3y2,i*h+h)
  k4y2 = ry2f(x[i,0]+.5*h*k3x1,x[i,1]+.5*h*k3y1,x[i,2]+.5*h*k3x2,x[i,3]+.5*h*k3y2,i*h+h)
