...
def ry1f(x1,y1,x2,y2,t):
  g = w*t
  return -a*y1+x1-x1**3+f*cos(g)
def rx2f(x1,y1,x2,y2,t):
  return y2
def ry2f(x1,y1,x2,y2,t):
  g = w*t
  return -a*y2+x2-x2**3+f*cos(g)+k*(x1-x2)
h = 0.01
n = 400000
x = zeros((n,5))
x[0,0]=0.5
...
a = x[:,0:2]
b = x[:,2:4]
v = zeros([n,2]); v[:,1] = x[:,0]; v[:,0] = x[:,2]
d =  zeros((n,2))
d[:,0] = x[:,0]-x[:,2] ; d[:,1] = x[:,1]-x[:,3]
print a[n-1], b[n-1]
g = Gnuplot.Gnuplot()
g1 = Gnuplot.Gnuplot()
g.title('Phase Space') ; g1.title('Phase Space')
g.xlabel('x') ; g.ylabel('y') ; g('set data style lines')
g1.xlabel('x') ; g1.ylabel('y') ; g1('set data style lines')
h1 = Gnuplot.Gnuplot() ; h2 = Gnuplot.Gnuplot()
h1.title('Time Series') ; h1.xlabel('t') ; h1.ylabel('x') ; h1('set data style lines') 
h2('set data style lines') ; h2.xlabel('t') ; h2.ylabel('y') 
dp = Gnuplot.Gnuplot() ; dp('set data style lines') ; dp.title('x2 vs x1') ; dp.xlabel('x1') ; dp.ylabel ('x2')
g.plot (a)
g1.plot (b)
h1.plot (x[:,0],x[:,2])
h2.plot (x[:,1],x[:,3])
...
while plt :
  ...
  dp.hardcopy('x2 vs x1.ps',enhanced=1, color=1)
  klo = float(raw_input('Do you want any more plots? 1/0 '))
  if klo == 0:
    plt = False